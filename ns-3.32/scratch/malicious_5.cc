...
int blahh=0;
std::ofstream outfile ("malicious_5_energy_output.txt");
std::ofstream outfile2 ("malicious_5_data.csv");
  ...
  if (tracing == true)
    {
      ...
      wifiPhy.EnableAsciiAll (ascii.CreateFileStream ("malicious_5_trace.tr"));
      wifiPhy.EnablePcap ("malicious_5", devices);
      // Trace routing tables
      Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("malicious_5.routes", std::ios::out);
      aodv.PrintRoutingTableAllEvery (Seconds (2), routingStream);
      Ptr<OutputStreamWrapper> neighborStream = Create<OutputStreamWrapper> ("malicious_5.neighbors", std::ios::out);
