...
int blahh=0;
std::ofstream outfile ("malicious_energy_output.txt");
std::ofstream outfile2 ("malicious_data.csv");
  ...
  if (tracing == true)
    {
      ...
      wifiPhy.EnableAsciiAll (ascii.CreateFileStream ("malicious_trace.tr"));
      wifiPhy.EnablePcap ("malicious", devices);
      // Trace routing tables
      Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("malicious.routes", std::ios::out);
      aodv.PrintRoutingTableAllEvery (Seconds (2), routingStream);
      Ptr<OutputStreamWrapper> neighborStream = Create<OutputStreamWrapper> ("malicious.neighbors", std::ios::out);
      aodv.PrintNeighborCacheAllEvery (Seconds (2), neighborStream);

      // To do-- enable an IP-level trace that shows forwarding events only
    }  
	
  Simulator::Schedule (Seconds (0), &GenerateTraffic, source0, 128,400, Seconds(0.5)); 
  Simulator::Schedule (Seconds (0.25), &GenerateTraffic, source1, 128,400, Seconds(0.5)); 
  Simulator::Schedule (Seconds (50), &GenerateTraffic, source0, 1024,1000, Seconds(0.001)); 
