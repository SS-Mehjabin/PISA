...
int blahh=0;
std::ofstream outfile ("malicious_3_energy_output.txt");
std::ofstream outfile2 ("malicious_3_data.csv");
  ...
  if (tracing == true)
    {
      ...
      wifiPhy.EnableAsciiAll (ascii.CreateFileStream ("malicious_3_trace.tr"));
      wifiPhy.EnablePcap ("malicious_3", devices);
      // Trace routing tables
      Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("malicious_3.routes", std::ios::out);
      aodv.PrintRoutingTableAllEvery (Seconds (2), routingStream);
      Ptr<OutputStreamWrapper> neighborStream = Create<OutputStreamWrapper> ("malicious_3.neighbors", std::ios::out);
