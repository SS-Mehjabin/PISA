...
int blahh=0;
std::ofstream outfile ("no_attack_energy_output.txt");
std::ofstream outfile2 ("no_attack_data.csv");
  ...
  if (tracing == true)
    {
      ...
      wifiPhy.EnableAsciiAll (ascii.CreateFileStream ("no_attack_trace.tr"));
      wifiPhy.EnablePcap ("no_attack", devices);
      // Trace routing tables
      Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("no_attack.routes", std::ios::out);
      aodv.PrintRoutingTableAllEvery (Seconds (2), routingStream);
      Ptr<OutputStreamWrapper> neighborStream = Create<OutputStreamWrapper> ("no_attack.neighbors", std::ios::out);
