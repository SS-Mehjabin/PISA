 ...
 * \ingroup examples
 * \brief Test script.EnablePcapAll
 * 
...
AodvExample::CreateNodes ()
{
  ...
  positionAlloc->Add (Vector (15.0, 12.0, 0.0));
  positionAlloc->Add (Vector (10.0, 5.0, 0.0));
  positionAlloc->Add (Vector (18.0, 7.0, 0.0));
  positionAlloc->Add (Vector (5.0, 8.0, 0.0));
  positionAlloc->Add (Vector (9.0, 10.0, 0.0));
  positionAlloc->Add (Vector (25.0, 10.0, 0.0));
  positionAlloc->Add (Vector (10.0, 15.0, 0.0));
  positionAlloc->Add (Vector (25.0, 15.0, 0.0));
  positionAlloc->Add (Vector (14.0, 17.0, 0.0));
  positionAlloc->Add (Vector (20.0, 17.0, 0.0));
...
AodvExample::CreateDevices ()
{
  ...
  if (pcap)
    {
      wifiPhy.EnablePcapAll (std::string ("sparse"));
    }
}
...
AodvExample::InstallInternetStack ()
{
  ...
  if (printRoutes)
    {
      Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("trust-sparse_routing_table_at_55.routes", std::ios::out);
      aodv.PrintRoutingTableAllAt (Seconds (55), routingStream);

      TrustManagerHelper trustManagerHelper;
      Ptr<OutputStreamWrapper> trustRoutingStream = Create<OutputStreamWrapper> ("Normaltrust-sparse_trust_table_at_55.routes", std::ios::out);
      trustManagerHelper.PrintTrustTableAllAt (Seconds (55), trustRoutingStream);

      Ptr<OutputStreamWrapper> routingStream3 = Create<OutputStreamWrapper> ("trust-sparse_routing_table_at_75.routes", std::ios::out);
      aodv.PrintRoutingTableAllAt (Seconds (75), routingStream3);

      TrustManagerHelper trustManagerHelper3;
      Ptr<OutputStreamWrapper> trustRoutingStream3 = Create<OutputStreamWrapper> ("Normaltrust-sparse_trust_table_at_75.routes", std::ios::out);
