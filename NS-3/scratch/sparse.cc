...
// Problem: No static route introducing 10.0.1.4 (Rpi) to simulated nodes.
  ...
  wifiPhy.EnablePcapAll (std::string ("A3P2_aodv"));

  for(int oo=0; oo<130; oo=oo+5)
    {
      TrustManagerHelper trustManagerHelper;
      string str1="A3P2Trust", str3=".routes";
      stringstream ss;
      ss << oo;
      string str2 = ss.str();
      ...
      for (uint32_t j = 0; j < 9; j++)
        {
          ...
          V4PingHelper ping (interfaces.GetAddress ( (9 - 1) - j));
          ping.SetAttribute ("Verbose",
                             BooleanValue (false));

          ApplicationContainer p = ping.Install (nodes.Get (i));
          p.Start (Seconds (0 + (i * 10)));
          p.Stop (Seconds (125) - Seconds (0.001));
        }

    }


  Simulator::Stop (Seconds (130.)); // Code runs for 80 seconds.
  Simulator::Run ();
  Simulator::Destroy ();
}
