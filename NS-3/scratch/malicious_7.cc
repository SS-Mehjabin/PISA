...
int blahh=0;
std::ofstream outfile ("malicious_7_energy_output.txt");
std::ofstream outfile2 ("malicious_7_data.csv");
  ...
  if (tracing == true)
    {
      ...
      wifiPhy.EnableAsciiAll (ascii.CreateFileStream ("malicious_7_trace.tr"));
      wifiPhy.EnablePcap ("malicious_7", devices);
      // Trace routing tables
      Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("malicious_7.routes", std::ios::out);
      aodv.PrintRoutingTableAllEvery (Seconds (2), routingStream);
      Ptr<OutputStreamWrapper> neighborStream = Create<OutputStreamWrapper> ("malicious_7.neighbors", std::ios::out);
